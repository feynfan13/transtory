class CrhDatabaseRouter(object):
    """
    ...
    def db_for_read(self, model, **hints):
        """Send all read operations on the current models to the current database."""
        if model._meta.app_label == self.app_label:
            return self.db_label
        return None

    def db_for_write(self, model, **hints):
        """Send all write operations on the current app models to the current database."""
        if model._meta.app_label == self.app_label:
